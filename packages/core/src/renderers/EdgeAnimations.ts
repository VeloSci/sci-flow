...
export const ANIMATION_CONFIG: Record<string, AnimConfig> = {
    ...
    'sine-orbit': {
        ...
        objectShape: 'circle',
        objectCount: 1,
        // sine wave perpendicular to path is done via animateTransform
    },
    'spin-x': {
        category: 'object',
        objectShape: 'x-mark',
    ...
    'draw-arrow': {
        category: 'compound',
        dashPart: 'draw',
        objectPart: 'arrow-travel',
    },
    'direction-pulse': {
        category: 'compound',
        dashPart: 'march',
        objectPart: 'arrow-travel', // fading arrow at center
    },
};
...
export function createTravelingObject(
    shape: AnimConfig['objectShape'],
    edgePathId: string,
    color: string,
    duration: string,
    delay: string,
    alternate: boolean,
    secondaryCssAnim?: string
): SVGGElement {
    ...
    switch (shape) {
        ...
        case 'radar': {
            ...
            addAnimateMotion(g, edgePathId, duration, delay, alternate);
            return g;
        }
        case 'circle':
        default: {
    ...
    if (secondaryCssAnim && shapeEl!) {
        shapeEl!.style.animation = `${secondaryCssAnim} ${duration} ${delay || '0s'} infinite`;
...
export function createAnimationObjects(
    ...
    for (let i = 0; i < count; i++) {
        ...
        const obj = createTravelingObject(
            ...
            count > 1 ? config.objectCssAnim : config.objectCssAnim
        );
