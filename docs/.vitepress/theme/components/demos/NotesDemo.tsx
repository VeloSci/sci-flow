...
const INIT_NODES: Node[] = [
  { id: 'a', type: 'default', position: { x: 80, y: 60 }, data: { title: 'API' }, inputs: {}, outputs: { output: { id: 'output', type: 'output', dataType: 'any' } } },
  { id: 'b', type: 'default', position: { x: 350, y: 60 }, data: { title: 'Cache' }, inputs: { input: { id: 'input', type: 'input', dataType: 'any' } }, outputs: { output: { id: 'output', type: 'output', dataType: 'any' } } },
  { id: 'c', type: 'default', position: { x: 80, y: 240 }, data: { title: 'DB' }, inputs: { input: { id: 'input', type: 'input', dataType: 'any' } }, outputs: { output: { id: 'output', type: 'output', dataType: 'any' } } },
  { id: 'd', type: 'default', position: { x: 350, y: 240 }, data: { title: 'Logger' }, inputs: { input: { id: 'input', type: 'input', dataType: 'any' } }, outputs: {} },
];
const INIT_EDGES = [
...
export function NotesDemo(_props: { theme?: string }) {
  ...
  const detectBundles = useCallback(() => {
    engine?.plugins.edgeBundler.detectBundles();
    setBundles(engine?.plugins.edgeBundler.getBundles().length ?? 0);
  }, [engine]);
  ...
  return (
    <div style={{ border: '1px solid #333', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {/* Render sticky notes as DOM overlays */}
        {notes.map(note => (
          <div key={note.id} style={{
            position: 'absolute', left: note.position.x, top: note.position.y,
            width: note.width, minHeight: 60, padding: '8px 10px',
            background: note.color, color: '#1a1a2e', borderRadius: 4,
